<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
      ...
      <div v-if="message" :class="messageClass" class="p-4 mt-4 text-sm rounded-md">
        {{ message }}
      </div>
      <div class="text-sm text-center">
        <router-link to="/register" class="font-medium text-indigo-600 hover:text-indigo-500">還沒有帳號？ 註冊</router-link>
      </div>
    </div>
  </div>
</template>
...
const handleLogin = async () => {
  ...
  const { success, message: resultMessage } = await auth.loginWithEmailAndPassword(
    email.value,
    password.value
  )
